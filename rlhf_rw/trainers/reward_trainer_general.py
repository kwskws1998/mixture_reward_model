...
def model_init_func(
    ...
    if load_local_folder_name is None:
        if bnb_config is not None:
            ...
            # prepare_model_for_kbit_training freezes ALL params; unfreeze
            # mixture_module so it gets trained.
            for name, param in model.named_parameters():
                if "mixture_module" in name:
                    param.requires_grad = True

        if peft_config is not None:
            model = get_peft_model(model, peft_config)
            # get_peft_model re-freezes all non-LoRA params; unfreeze
            # mixture_module again.
            for name, param in model.named_parameters():
                if "mixture_module" in name:
                    param.requires_grad = True
        ...
        if getattr(base, 'use_mixture_token', False):
            mix_path = load_local_folder_name + "/mixture_module.bin"
            if _os.path.isfile(mix_path):
                base.mixture_module.load_state_dict(
                    torch.load(mix_path, map_location='cpu')
                )
                print(f"Mixture module loaded (K={getattr(base, 'mixture_K', '?')}, "
                      f"cov_type={getattr(base, 'mixture_cov_type', '?')})")
...
class CustomRewardTrainer(RewardTrainer):
    def save_model(self, output_dir=None, _internal_call=True):
        ...
        if getattr(base, 'use_mixture_token', False):
            torch.save(
                base.mixture_module.state_dict(),
                output_dir + "/mixture_module.bin",
            )
            print(f"Mixture module saved (K={getattr(base, 'mixture_K', '?')}, "
                  f"cov_type={getattr(base, 'mixture_cov_type', '?')})")
...
class RewardTrainerConstructorGeneral(RewardTrainerConstructor):
    ...
    def eval_model(self, folder_name, mode="all"):
        ...
        if self.dataset_name == "allenai/reward-bench":
            self.load_dataset_rewardbench(
                max_length=self.max_length, max_tokens=self.max_tokens, model="subsets"
            )
            if mode == "all":
                results = {}
    ...
    def set_trainer(self, save_folder="./reward_model"):
        ...
        trainer_args = {
            ...
            # peft_config intentionally omitted: model is already PEFT-wrapped by
            # model_init_func. Passing peft_config again causes RewardTrainer to call
            # prepare_model_for_kbit_training a second time, re-freezing redistribution params.
        }
