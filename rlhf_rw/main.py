...
if __name__ == "__main__":
    ...
    # ── Mixture-token (per-response GMM) — replaces deprecated redistribution ──
    parser.add_argument(
        "--use_mixture_token",
        help="enable per-response GMM summary token (true/false)",
        default=False,
    )
    parser.add_argument(
        "--mixture_K",
        type=int,
        help="GMM components per response (default 3)",
        default=3,
    )
    parser.add_argument(
        "--mixture_cov_type",
        help="GMM covariance type: full | diag | spherical | tied",
        default="diag",
    )
    parser.add_argument(
        "--mixture_proj_hidden",
        type=int,
        help="hidden size of mixture-token projector MLP",
        default=128,
    )
    parser.add_argument(
        "--mixture_dropout",
        type=float,
        help="dropout in mixture-token projector",
        default=0.1,
    )
    parser.add_argument(
        "--mixture_log_transform",
        help="apply log1p to ET features before GMM fit (true/false)",
        default=True,
    )
    parser.add_argument(
    ...
    use_mixture_token = str(args.use_mixture_token).lower() == "true"
    mixture_K = int(args.mixture_K)
    mixture_cov_type = str(args.mixture_cov_type).lower()
    mixture_proj_hidden = int(args.mixture_proj_hidden)
    mixture_dropout = float(args.mixture_dropout)
    mixture_log_transform = str(args.mixture_log_transform).lower() == "true"
    ...
    run_rewardbench_flag = str(args.run_rewardbench).lower() == "true"
    ...
    if mode == "train":
        ...
        if concat is False or use_softprompt is True:
            torch.save(
                ...
                folder_name_unique_path + "/fixations_projector_state_dict.bin",
            )
            torch.save(
                reward_trainer.model.base_model.model.norm_layer_fix.state_dict(),
                folder_name_unique_path + "/layer_norm_state_dic.bin",
            )
        ...
        # Auto RewardBench eval after training
        if run_rewardbench_flag:
            print("\n[main] --run_rewardbench True — starting RewardBench eval...")
